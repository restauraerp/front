...
import { BookOpen, Receipt, Percent, Tag } from 'lucide-react';

export default function AccountingDashboard() {
  const [stats, setStats] = useState({ ledgers: 0, expenses: 0, taxes: 0, headers: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function loadStats() {
      try {
        const [ledgerRes, expenseRes, taxRes, headerRes] = await Promise.all([
